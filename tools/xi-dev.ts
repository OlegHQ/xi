...
if (buildOnly || await sourcesAreNewer(output)) {
  const { createXiSolidBuildPlugin } = await import('./solid-build-plugin');
...
async function sourcesAreNewer(target: string): Promise<boolean> {
  ...
  const changed = Bun.spawnSync([
    'find', 'apps/xi/src', 'packages', 'package.json', 'bun.lock', 'tsconfig.json', 'tools/xi-dev.ts', 'tools/solid-build-plugin.ts',
