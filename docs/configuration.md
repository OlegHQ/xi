...
| `editor.file-explorer.hidden` | `true` in Xi (`false` in Helix) | master | Deliberate default incompatibility | Files hides dotfiles by default; `false` reveals them. |
...
| `editor.file-explorer.{parents,ignore,git-ignore,git-global,git-exclude}` | `true` in Xi (`false` in Helix) | master | Deliberate default incompatibility | Files hides ignored entries by default; each ignore source remains independently configurable and the session toggle can reveal them. |
