...
The isolated declarative CLI replaces the real status-message surface, sharing the production
...
surface, not a complete declarative editor or panel-parity claim.
...
common first-key failure. The production migration below selects Solid. The React candidate
remains historical comparison evidence; Solid preserves the same ownership, parity and
performance requirements.

## End state

One Solid root owns shell composition and the lifetime of UI surfaces. Components read
...
surface-specific formatting and commands local. Use Solid's declarative Core elements;
...
hit testing. Do not create a Solid node per character, copy buffer text into Solid state,
...
Solid host itself ultimately draws through the native OpenTUI renderer too.
...
| Per-panel `*Renderable` classes with repeated paint/lifecycle plumbing | Solid panels using shared layout/rows; delete each old surface when its replacement passes |
...
| Ordered async key drain and WorkbenchInputRouter | Retain: command order and overlay focus are semantic contracts, not Solid reconciliation responsibilities |
...
| Platform setup, suspend/resume, terminal restoration | Small terminal adapter outside Solid, with one teardown owner |

Solid does **not** replace input serialization or make service work nonblocking. Each
keypress must still reach the engine in order, including across asynchronous commands.
Do not move this ordering into state effects or transition queues. Solid and the custom
...
- Expose the viewport using the pinned Solid host's custom-renderable mechanism; verify
  mount/update/dispose ownership with a focused check before shell extraction. Do not
  imperatively attach children Solid can later remove.
- Use Solid signals/effects on existing read ports. Unchanged snapshots must have stable
  identity. If a current getter allocates every read, fix caching at that owner; never
  build a duplicate mutable store to accommodate Solid.
...
- Keep the existing focus priority and semantic input router. Solid mouse handlers
...
- Preserve terminal-cell versus document-coordinate distinctions. Cursor-relative
  ...
  They must not read speculative Solid layout or accept stale pointer hit maps.
...
- Solid/Core must resolve to one pinned patched Core instance, including the compiled
...
1. **T136: establish a trustworthy adoption comparison.** Keep the terminal-key regression passing, validate the observer against known frames and exact saved-state checks,
   ...
   private/Core/Solid paths against pinned clean Neovim. Keep all 4/8/16/25 ms limits,
...
2. **T137: one root and declarative shell.** Extract viewport-only drawing from
   ...
   Solid. Delete the corresponding imperative paint and synchronization. Preserve
...
4. **T139: overlays and final retirement.** Migrate command line, picker, completion,
   ...
   Reuse existing semantic controllers rather than reimplementing behavior in Solid.
...
Compile production Solid/reconciler branches and disable development-tool loading in
...
behavior. T136 measures the minified production Solid candidate from
