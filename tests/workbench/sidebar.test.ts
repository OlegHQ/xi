...
// T-SIDEBAR-01: Files starts expanded; Outline starts collapsed when the outline model has no
// symbols yet.
{
  const outline = new FakeOutline();
  const sidebar = new SidebarController({ outline });
  const model = sidebar.readModel();
  assert.equal(model.sections.find((section) => section.id === 'files')?.expanded, true, 'T-SIDEBAR-01a Files is expanded by default');
