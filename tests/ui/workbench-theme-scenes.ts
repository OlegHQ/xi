...
export const themeScenes: Record<string, { readonly options: Omit<OpenTuiWorkbenchOptions, 'dispatchKey'>; readonly marker: string }> = {
  ...
  outline: { marker: 'auditOutline', options: { outline: { isOpen: open, read: read({ state: 'ready', symbols: [{ id: 'symbol', name: 'auditOutline', detail: 'function', kind: 12, children: [] }], message: undefined }) } } },
  ...
  prefix: { marker: 'auditPrefix', options: { prefixHelp: read({ registryGeneration: 1, focusGeneration: 1, configGeneration: 1, targetId: 'view', pendingKeys: ['<Space>'], compactHint: 'auditPrefix', hints: names.map(name => ({ kind: 'mapping', keys: [name[0]!], keyLabel: name[0]!, sequence: ['<Space>', name[0]!], title: `auditPrefix ${name}`, description: 'Example action', commandId: 'files.pick', aliases: [], available: name !== 'inactive', disabledReason: name === 'inactive' ? 'Unavailable here' : undefined })) }) } },
