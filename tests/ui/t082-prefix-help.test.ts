...
import { defineCommandRegistration, type CommandDescriptorCandidate, type CommandInvocationRef, type CommandSchema } from '../../packages/contracts/src/index';
...
// Live-router equivalent of the deleted WorkbenchInputDispatcher's binding table: prefix
// help is built the same way the production router builds it (`buildPrefixHelpReadModel`
// straight from the registry and a plain binding list -- see
// packages/workbench/input/router.ts's own `readPrefixHelp` closure), just with a fixture
// binding list instead of the router's currently-empty one, so this test still exercises
// mapping hints/aliases/capability availability.
const editorTargetId = 'editor-t082';
const bindingCommand = (commandId: string): CommandInvocationRef => ({ kind: 'command', id: id(commandId) });
const bindings: readonly PrefixHelpBinding[] = Object.freeze([
  Object.freeze({ targetId: editorTargetId, keys: Object.freeze(['d', 'w']), command: bindingCommand('xi.t082.delete-word'), contexts: Object.freeze(['editor', 'normal']), description: 'Delete the next word' }),
  Object.freeze({ targetId: editorTargetId, keys: Object.freeze(['<Space>', 'm', 'a']), command: bindingCommand('xi.t082.leader'), contexts: Object.freeze(['editor', 'normal']), description: 'Add next occurrence' }),
  Object.freeze({ targetId: editorTargetId, keys: Object.freeze(['g', 'd']), command: bindingCommand('xi.t082.capability'), contexts: Object.freeze(['editor', 'normal']), description: undefined }),
]);
let configGeneration = 1;
const focusGeneration = 0;
const source: PrefixHelpSource = {
  readGenerations(): PrefixHelpGenerations {
    return Object.freeze({ registryGeneration: registry.snapshot.generation, configGeneration, focusGeneration });
  },
  readPrefixHelp(request: PrefixHelpRequest) {
    // The deleted WorkbenchInputDispatcher derived this from the active FocusGraph target's
    // own contexts; this fixture has one fixed editor target, so its contexts stand in.
    const withAvailability: PrefixHelpRequest = request.availability === undefined
      ? { ...request, availability: { contexts: ['editor', 'normal'], capabilities: [] } }
      : request;
    return buildPrefixHelpReadModel({ request: withAvailability, registry, registrySnapshot: registry.snapshot, focusGeneration, bindings });
  },
};
...
controller.schedule({ targetId: editorTargetId, pendingKeys: ['d'], parserContinuations: parserD, configGeneration });
...
const unavailable = buildPrefixHelpReadModel({
  request: { targetId: editorTargetId, pendingKeys: ['g'], parserContinuations: [], configGeneration, availability: { contexts: ['editor'], capabilities: [] } },
  registry,
...
// The live router (not the deleted WorkbenchInputDispatcher) resolves the actual key-to-command
// dispatch; this only proves the passive help controller's own contract -- completing a command
// (however it was resolved) clears any scheduled help before the delayed paint fires, and
// leaves no dangling timer, regardless of how many prefix keys were pending.
controller.schedule({ targetId: editorTargetId, pendingKeys: ['d'], parserContinuations: parserD, configGeneration });
assert.equal(clock.pendingCount, 1, 'E20-TIMING-04 command prefix can be pending while help is scheduled');
...
controller.schedule({ targetId: editorTargetId, pendingKeys: ['f'], parserContinuations: parserLiteral, configGeneration });
clock.fireAll();
...
controller.schedule({ targetId: editorTargetId, pendingKeys: ['r'], parserContinuations: parserLiteral, configGeneration });
clock.fireAll();
