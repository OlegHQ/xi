...
import { WorkbenchRenderable, LIGHT_WORKBENCH_THEME, calculatePaintRanges } from '../../packages/ui/src/index';
...
function makeFixture(): { readonly workbench: WorkbenchReadPort; readonly snapshot: DocumentSnapshot; readonly view: WorkbenchViewSnapshot } {
  ...
  const members = [{ id: primary, kind: 'normal-cursor' as const, direction: 'forward' as const, anchor: { kind: 'character' as const, offset: offset(0), after: offset(1) }, head: { kind: 'character' as const, offset: offset(0), after: offset(1) }, desiredColumn: desired }];
...
function findSpanText(frame: ReturnType<Awaited<ReturnType<typeof createTestRenderer>>['captureSpans']>, text: string): { readonly fg: { r: number; g: number; b: number } } | undefined {
...
async function keywordAndPlainCellsDifferByColor(): Promise<void> {
  ...
  assert.ok(keywordSpan !== undefined, 'T053-PAINT-01 the keyword glyph is present in the painted frame');
  assert.ok(stringSpan !== undefined, 'T053-PAINT-01b the string-classified glyph is present in the painted frame');
  assert.ok(plainSpan !== undefined, 'T053-PAINT-02 an unclassified glyph is present in the painted frame');
  ...
  assert.equal(keywordSpan?.fg.r, expectedKeywordColor.r, 'T053-PAINT-03 the keyword cell uses the theme keyword color (r)');
  assert.equal(keywordSpan?.fg.g, expectedKeywordColor.g, 'T053-PAINT-03 the keyword cell uses the theme keyword color (g)');
  assert.equal(keywordSpan?.fg.b, expectedKeywordColor.b, 'T053-PAINT-03 the keyword cell uses the theme keyword color (b)');
  assert.equal(stringSpan?.fg.r, expectedStringColor.r, 'T053-PAINT-03b the string-classified cell uses the theme string color (r)');
  const plainColor = parseColor(LIGHT_WORKBENCH_THEME.foreground);
  assert.equal(plainSpan?.fg.r, plainColor.r, 'T053-PAINT-04 an unclassified cell keeps the plain foreground (r)');
  assert.notEqual(keywordSpan?.fg.r, plainSpan?.fg.r, 'T053-PAINT-05 keyword and plain cells are painted with different colors');
  assert.notEqual(stringSpan?.fg.r, plainSpan?.fg.r, 'T053-PAINT-05b string-classified and plain cells are painted with different colors');
...
async function staleVersionReadPaintsPlain(): Promise<void> {
  ...
  const plainColor = parseColor(LIGHT_WORKBENCH_THEME.foreground);
  assert.equal(keywordSpan?.fg.r, plainColor.r, 'T053-PAINT-07 a stale-version syntax read paints plain, not the syntax color (r)');
  assert.equal(keywordSpan?.fg.g, plainColor.g, 'T053-PAINT-07 a stale-version syntax read paints plain, not the syntax color (g)');
  assert.equal(keywordSpan?.fg.b, plainColor.b, 'T053-PAINT-07 a stale-version syntax read paints plain, not the syntax color (b)');
  setup.renderer.destroy();
}
...
async function firstPaintIsPlainSecondPaintIsColored(): Promise<void> {
  ...
  const coldRead: SyntaxRead = { documentVersion: fixture.snapshot.version, spansInRange: () => [] };
  const drainedRead: SyntaxRead = {
    documentVersion: fixture.snapshot.version,
    spansInRange: (start, end) => spans.filter((span) => span.start < end && span.end > start),
  };
  let drained = false;
  const port: SyntaxReadPort = { readSyntax: (documentId) => documentId === DOCUMENT_ID ? (drained ? drainedRead : coldRead) : undefined };
  const setup = await createTestRenderer({ width: 80, height: 24, bufferedOutput: 'memory', gatherStats: true });
  const viewport = new WorkbenchRenderable(setup.renderer.root.ctx, {
    workbench: fixture.workbench,
    fileLabel: 'editor.ts',
  ...
  assert.ok(firstKeywordSpan !== undefined, 'T053-PAINT-08 the keyword glyph is present on the first paint');
  assert.equal(firstKeywordSpan?.fg.r, plainColor.r, 'T053-PAINT-09 the first paint (uncached window) is plain, not colored');
  ...
  assert.ok(secondKeywordSpan !== undefined, 'T053-PAINT-10 the keyword glyph is present on the second paint');
  assert.equal(secondKeywordSpan?.fg.r, expectedKeywordColor.r, 'T053-PAINT-11 the second paint (after the background window resolves) is colored');
  setup.renderer.destroy();
}
...
async function staleReadKeepsUnaffectedRowColoredThenFollowUpRepaintsOnlyChangedRow(): Promise<void> {
  ...
  const makeView = (): WorkbenchViewSnapshot => {
    ...
    const selections = createSelectionSet(snapshot, {
      ...
      members: [{ id: primary, kind: 'normal-cursor', direction: 'forward', anchor: { kind: 'character', offset: offset.value, after: after.value }, head: { kind: 'character', offset: offset.value, after: after.value } }],
    });
    if (!selections.ok) throw new Error(`T053-STALE-selection:${selections.error.kind}`);
    return { session: { viewId: VIEW_ID, documentId: DOCUMENT_ID, documentVersion: snapshot.version, selections: selections.value.selectionSet, mode: 'normal' }, document: snapshot, selections: selections.value.selectionSet, scrollTop: 0, scrollLeft: 0 };
  };
  let view = makeView();
  ...
  const workbench: WorkbenchReadPort = { activeViewId: VIEW_ID, readView: (viewId) => viewId === VIEW_ID ? view : undefined, readDocument: (viewId) => viewId === VIEW_ID ? workbenchDocument : undefined };

  const keywordSpans: readonly SyntaxSpan[] = Object.freeze([Object.freeze({ start: 0, end: 5, kind: 'keyword' as const })]);
  let currentRead: SyntaxRead = { documentVersion: snapshot.version, spansInRange: (start, end) => keywordSpans.filter((span) => span.start < end && span.end > start) };
  const port: SyntaxReadPort = { readSyntax: (documentId) => documentId === DOCUMENT_ID ? currentRead : undefined };

  const setup = await createTestRenderer({ width: 80, height: 24, bufferedOutput: 'memory', gatherStats: true });
  const viewport = new WorkbenchRenderable(setup.renderer.root.ctx, { workbench, fileLabel: 'editor.ts', theme: LIGHT_WORKBENCH_THEME, syntax: port });
  setup.renderer.root.add(viewport);
  await setup.renderOnce();

  const expectedKeywordColor = parseColor(LIGHT_WORKBENCH_THEME.syntax?.keyword as string);
  const initialKeyword = findSpanText(setup.captureSpans(), 'onst');
  assert.equal(initialKeyword?.fg.r, expectedKeywordColor.r, 'T053-STALE-00 the keyword is colored before any edit');
  ...
  const committed = document.commit({ documentId: document.id, expectedVersion: document.version, edits: [{ start: end.value, end: end.value, text: 'X' }], origin: 'vim', undoGroup: group.value });
  ...
  assert.equal(staleKeyword?.fg.r, expectedKeywordColor.r, 'T053-STALE-01 a typed character elsewhere does not drop color on the unchanged keyword row');
  const afterStale = viewport.lastFrame;
  assert.ok(afterStale?.frame !== undefined && afterStale.view !== undefined, 'T053-STALE-02 a frame is projected for the stale-read paint');

  // The background parse now lands: a new read object, current for the new version.
  currentRead = { documentVersion: snapshot.version, spansInRange: (start, endOffset) => keywordSpans.filter((span) => span.start < endOffset && span.end > start) };
  viewport.refresh();
  await setup.renderOnce();
  const freshKeyword = findSpanText(setup.captureSpans(), 'onst');
  assert.equal(freshKeyword?.fg.r, expectedKeywordColor.r, 'T053-STALE-03 the keyword stays colored once the fresh parse lands');
  const afterFresh = viewport.lastFrame;
  assert.ok(afterFresh?.frame !== undefined && afterFresh.view !== undefined, 'T053-STALE-04 a frame is projected for the fresh-read paint');

  const ranges = calculatePaintRanges(afterStale, afterFresh!.frame!, afterFresh!.view!, undefined, undefined, false, currentRead, staleRead);
  const repaintedRows = ranges.reduce((sum, range) => sum + (range.end - range.start), 0);
  assert.ok(repaintedRows <= 2, `T053-STALE-05 the fresh parse landing repaints <=2 rows (the edited line), got ${repaintedRows} (ranges=${JSON.stringify(ranges)})`);
...
console.log('T053 syntax paint passed: keyword/plain color differentiation, stale-version plain fallback, plain-then-colored across two paints, and stale-fallback/bounded-repaint fixtures');
