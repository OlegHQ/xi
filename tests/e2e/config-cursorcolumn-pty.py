#!/usr/bin/env python3
"""Prove editor.cursorcolumn changes the launched editor column paint."""
...
with tempfile.TemporaryDirectory(prefix="xi-cursorcolumn-pty-") as temporary:
    root = Path(temporary)
    config = root / ".config" / "xi" / "config.toml"
    config.parent.mkdir(parents=True)
    config.write_text('schema-version = 1\ntheme = "xi-dark"\n[editor]\ncursorcolumn = true\n', encoding="utf-8")
    source = root / "cursorcolumn.txt"
    source.write_text("abc\nxyz\n", encoding="utf-8")

    master, slave = pty.openpty()
    fcntl.ioctl(slave, termios.TIOCSWINSZ, struct.pack("HHHH", 14, 60, 0, 0))
    environment = os.environ.copy()
    environment.update({"HOME": temporary, "TERM": "xterm-256color", "COLORTERM": "truecolor", "XI_UI_TEST_MARKERS": "1"})
    ...
    try:
        ...
        os.write(master, b"l")
        captured.extend(read_until(master, b"\x1b[48;2;49;50;68m", 3))
        if b"\x1b[48;2;49;50;68m" not in captured:
            raise SystemExit(f"cursorcolumn did not paint the active column: {captured[-4000:]!r}")
        os.write(master, b"q")
...
print("Config cursorcolumn PTY passed: launched Xi painted the configured active column.")
