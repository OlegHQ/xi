...
with tempfile.TemporaryDirectory(prefix="xi-statusline-pty-") as temporary:
    ...
    config.write_text("schema-version = 1\n[editor.statusline]\nleft = [\"mode\", \"separator\", \"file-type\", \"register\", \"current-working-directory\"]\nseparator = \"~\"\nmode.normal = \"NORMX\"\n", encoding="utf-8")
    ...
    try:
        ...
        expected = f"NORMX ~text \"  {ROOT}".encode()
        if expected not in captured:
            raise SystemExit(f"configured statusline was not rendered: {captured[-4000:]!r}")
        os.write(master, b"q")
...
print("Config statusline PTY passed: launched Xi rendered mode, separator and current-working-directory.")
