...
with tempfile.TemporaryDirectory(prefix="xi-statusline-pty-") as temporary:
    ...
    try:
        ...
        if expected not in screen.row_text(14):
            raise SystemExit(f"configured statusline was not rendered: {screen.row_text(14)!r}")
...
print("Config statusline PTY passed: launched Xi followed space/tab EditorConfig across buffers, omitted UTF-8 and counted one emoji scalar.")
