...
with tempfile.TemporaryDirectory(prefix="xi-trust-prompt-pty-") as temporary:
    ...
    try:
        ...
        if b"XI_WORKBENCH_READY" not in captured:
            raise SystemExit(f"Xi did not start: {captured[-2500:]!r}")
        screen = Screen(14, 100)
        screen.feed(captured)
        rows = "\n".join(screen.row_text(row) for row in range(1, 15))
        if "Trust workspace" not in rows or "Never trust workspace" not in rows:
            raise SystemExit(f"trust prompt absent: {rows!r}")
        os.write(master, b"\x1b")
        captured.clear()
        read_for(master, captured, 8.5)
        screen.feed(captured)
        rows = "\n".join(screen.row_text(row) for row in range(1, 15))
    ...
    for choice, keys in (("trust", b"\r"), ("never", b"j\r")):
        ...
        try:
            read_for(master, captured, 2)
            if b"XI_WORKBENCH_READY" not in captured or b"Trust workspace" not in captured:
                raise SystemExit(f"{choice} prompt absent: {captured[-2500:]!r}")
            os.write(master, keys)
            read_for(master, captured, 8.5)
            records = list((home / ".local/share/xi/workspace_trust").glob("*.json"))
        ...
        try:
            read_for(master, captured, 2)
            if b"XI_WORKBENCH_READY" not in captured or b"Trust workspace" in captured:
    ...
    try:
        read_for(master, captured, 2)
        if b"XI_WORKBENCH_READY" not in captured or b"Trust workspace" not in captured:
            raise SystemExit(f"restricted LSP without local config did not prompt: {captured[-2500:]!r}")
