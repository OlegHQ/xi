...
let nextTimestamp = 0;
const event = (phase: PointerEvent['phase'], viewId: string, row: number, column: number, overrides: Partial<PointerEvent> = {}): PointerEvent => ({ phase, viewId, cell: { row, column }, button: phase === 'wheel' ? null : 0, modifiers, wheelDelta: 0, frameId: 1, timestampMilliseconds: (nextTimestamp += 1), ...overrides });
