...
const fixturePath = resolve(root, 'tests/vim/t020/t020-fixtures.json');
const tracePath = resolve(root, 'tests/vim/t020/t020-oracle-traces.json');
const catalog = JSON.parse(await readFile(fixturePath, 'utf8')) as FixtureCatalog;
assert.equal(catalog.schemaVersion, 1, 'T020-ORACLE-CATALOG-01 fixture schema is supported');
...
for (const fixture of catalog.fixtures) {
  ...
  assert.equal(result.snapshots.length, fixture.steps.length,
    `T020-ORACLE-SNAPSHOT-01 ${fixture.id} has one snapshot per key barrier`);
...
const trace: OracleTrace = {
  ...
  fixtures,
  note: 'Pinned Neovim 0.12.4 fixture evidence for T020 only; compatibility inventory remains owned by T061.',
};

if (process.argv.includes('--record')) {
  await writeFile(tracePath, `${JSON.stringify(trace, null, 2)}\n`, 'utf8');
  console.log(`RECORDED ${fixtures.length} T020 fixtures to ${tracePath}`);
} else {
  const expected = JSON.parse(await readFile(tracePath, 'utf8')) as OracleTrace;
  assert.deepEqual(expected, trace, 'T020-ORACLE-TRACE-01 pinned snapshots match the catalog');
  console.log(`T020 oracle passed ${fixtures.length} fixtures / ${fixtures.reduce((sum, fixture) => sum + fixture.snapshots.length, 0)} snapshots`);
}
