...
export interface HostNavigationProvider {
  file(path: string, line?: number, cancellation?: CancellationToken): Promise<Result<HostLocation, HostNavigationFailure>>;
  tag(name: string, cancellation?: CancellationToken): Promise<Result<readonly HostLocation[], HostNavigationFailure>>;
}
...
export class HostNavigationController implements Disposable {
  ...
  async openFile(path: string, line?: number, cancellation?: CancellationToken): Promise<Result<HostNavigationResult, HostNavigationFailure>> { return this.commit(await this.#provider.file(path, line, cancellation)); }
  async openTag(name: string, cancellation?: CancellationToken): Promise<Result<HostNavigationResult, HostNavigationFailure>> { const generation = ++this.#generation; const result = await this.#provider.tag(name, cancellation); if (generation !== this.#generation) return { ok: false, error: { kind: 'stale', message: 'tag request is stale' } }; if (!result.ok) return result; if (result.value.length === 0) return { ok: false, error: { kind: 'missing', message: `tag ${name} was not found` } }; if (result.value.length > 1) return { ok: false, error: { kind: 'ambiguous', message: `tag ${name} has multiple matches` } }; return this.commit({ ok: true, value: result.value[0]! }); }
