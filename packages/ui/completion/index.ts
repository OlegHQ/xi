...
export interface CompletionItemRead { readonly id: string; readonly label: string; readonly kind?: number; readonly detail?: string; readonly documentation?: string; }
...
// LSP CompletionItemKind values 1–25; the same Nerd Font already used by Explorer.
const KIND_ICONS = ['󰉿', '󰊕', '󰊕', '󰆧', '󰀫', '󰀫', '󰠱', '󰕘', '󰏗', '󰀫', '󰑭', '󰎠', '󰕘', '󰌋', '󰘍', '󰏘', '󰈙', '󰈇', '󰉋', '󰎠', '󰏿', '󰙅', '󰫧', '󰆕', '󰊄'] as const;
export function formatCompletionLines(model: CompletionReadModel, width: number, maxRows = 8): readonly string[] {
  ...
  for (const item of model.items.slice(0, Math.max(0, maxRows - 1))) {
    const icon = item.kind === undefined ? '󰌋' : KIND_ICONS[item.kind - 1] ?? '󰌋';
    rows.push(`${item.id === model.selectedId ? '▸' : ' '} ${icon} ${item.label}${item.detail === undefined ? '' : ` — ${item.detail}`}`.slice(0, Math.max(1, width)));
  }
