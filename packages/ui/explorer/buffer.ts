...
export function createDirectoryExplorerRead(read: Pick<ExplorerBufferController, 'model' | 'generation' | 'treeRows' | 'subscribe'>, tree: ExplorerReadPort): ExplorerReadPort {
  const project = (): ExplorerReadModel => {
    ...
    const labels = new Map(base.visibleRows.map(row => [row.nodeId, row.label ?? original.get(row.nodeId)?.name]));
    ...
    return { ...base, generation: editing.generation + base.generation, nodes, selectedId,
      ...
      edit: { mode: editing.mode, cursorColumn: selected?.cursorColumn ?? editing.cursorColumn, visualIds: rows.filter((row) => row.visual).map((row) => row.id), dirty: editing.dirty, prompt: editing.prompt, ...(editing.scroll === undefined ? {} : { scroll: editing.scroll }), pendingIds: rows.filter((row) => row.line !== undefined && (row.nodeId === undefined || row.name !== labels.get(row.nodeId) || row.path.slice(0, row.path.lastIndexOf('/')) !== row.bufferPath)).map((row) => row.id), ...(editing.reviewLines === undefined ? {} : { review: { lines: editing.reviewLines, selectedIndex: editing.selectedIndex, confirm: editing.reviewConfirm, busy: editing.busy } }) },
    };
  };
