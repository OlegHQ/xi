...
export function createDirectoryExplorerRead(read: Pick<ExplorerBufferController, 'model' | 'generation' | 'treeRows' | 'subscribe'>, tree: ExplorerReadPort): ExplorerReadPort {
  const project = (): ExplorerReadModel => {
    ...
    return { ...base, generation: editing.generation + base.generation, nodes, selectedId,
      ...
      edit: { mode: editing.mode, cursorColumn: selected?.cursorColumn ?? editing.cursorColumn, visualIds: rows.filter((row) => row.visual).map((row) => row.id), dirty: editing.dirty, prompt: editing.prompt, ...(editing.scroll === undefined ? {} : { scroll: editing.scroll }), pendingIds: rows.filter((row) => row.pending).map((row) => row.id), ...(editing.reviewLines === undefined ? {} : { review: { lines: editing.reviewLines, selectedIndex: editing.selectedIndex, choice: editing.reviewChoice, error: editing.reviewError, busy: editing.busy } }) },
    };
  };
