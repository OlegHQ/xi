...
export interface ExplorerVisibleRow { readonly nodeId: string; readonly depth: number; readonly kind: ExplorerNodeKind; readonly selected: boolean; }
export interface ExplorerReadModel { readonly contractVersion: 1; readonly generation: number; readonly roots: readonly string[]; readonly nodes: readonly ExplorerNode[]; readonly visibleRows: readonly ExplorerVisibleRow[]; readonly selectedId: string | undefined; readonly filter: string; readonly includeHidden: boolean; readonly includeIgnored: boolean; readonly focused: boolean; readonly state: 'ready' | 'loading' | 'empty' | 'error'; readonly message: string | undefined; }
...
function nodeLine(node: ExplorerNode, depth: number, ascii: boolean): string {
...
export function formatExplorerLines(model: ExplorerReadModel, width: number, maxRows: number, showHeader = true, scrollOffset = 0, ascii = false): readonly string[] {
  ...
  for (const row of model.visibleRows.slice(Math.max(0, Math.trunc(scrollOffset)))) { if (lines.length >= safeRows) break; const node = model.nodes.find(candidate => candidate.id === row.nodeId); if (node !== undefined) lines.push(nodeLine(node, row.depth, ascii).slice(0, safeWidth)); }
