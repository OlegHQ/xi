...
export class ComparisonEditor {
  ...
  paint(buffer: OptimizedBuffer, model: DiffViewReadModel, view: WorkbenchViewSnapshot, rect: { readonly x: number; readonly y: number; readonly width: number; readonly height: number }, theme: WorkbenchTheme, syntax?: SyntaxReadPort, ascii = false, colorMode: EditorColorMode = 'truecolor'): ComparisonPaint {
    ...
    const muted = parseColor(theme.muted);
    const accent = parseColor(theme.accent);
    const added = parseColor(theme.diffAdded ?? (background.r < 0.5 ? '#243D2B' : '#E4F3E8'));
    const removed = parseColor(theme.diffRemoved ?? (background.r < 0.5 ? '#48282D' : '#F8E5E8'));
    ...
    buffer.drawText(header.slice(0, rect.width), rect.x, rect.y, accent, parseColor(theme.surface));
    if (split) {
