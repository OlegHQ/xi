...
export class ContextMenuStore {
  ...
  select(index: number): void { const state = this.#state; if (state === undefined || index === state.selectedIndex || !Number.isSafeInteger(index) || state.items[index]?.enabled !== true) return; this.#state = Object.freeze({ ...state, selectedIndex: index }); this.notify(); }
